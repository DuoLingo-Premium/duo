...
const Signup: React.FC = () => {
  ...
  const validatePassword = () => {
    // const passwordRules = /^(?=.*[A-Z])(?=.*\d)[A-Za-z\d]{8,}$/; // Password must be at least 8 characters, include one uppercase letter, and one number
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const response = await useAxios("/api/user/signUp", "POST", formData);

    if (response?.ok) {
      toast.success("Signup successful! Please log in.");
      setTimeout(() => navigate.push("/auth/signIn"), 2000);
    } else {
